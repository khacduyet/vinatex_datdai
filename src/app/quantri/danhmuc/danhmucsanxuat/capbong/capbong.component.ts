...
@Component({
  selector: 'app-capbong',
  templateUrl: './capbong.component.html',
  styleUrls: ['./capbong.component.css']
})
export class CapbongComponent implements OnInit {
  ...
  GetListdm(reset?){
    ...
    this._services.GetListdmCapBong(this.dataSearch).subscribe((res:any)=>{
  ...
  add(){
    ...
    modalRef.componentInstance.type = 'capbong';
    modalRef.componentInstance.title = 'Thêm mới danh mục cấp bông';
  ...
  edit(item){
    ...
    modalRef.componentInstance.title = 'Cập nhật danh mục cấp bông';
  ...
  delete(item){
    ...
    modalRef.result.then(res=>{
      this._services.DeletedmCapBong(item).subscribe((res: any) => {
        if (res) {
          if (res.State === 1) {
